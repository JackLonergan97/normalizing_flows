...
def CDMFromEmulator(z_lens, z_source, concentration_model_subhalos='DIEMERJOYCE19', kwargs_concentration_model_subhalos = {}, concentration_model_fieldhalos='DIEMERJOYCE19', kwargs_concentration_model_fieldhalos = {}, truncation_model_subhalos='TRUNCATION_ROCHE_GILMAN2020', kwargs_truncation_model_subhalos = {}, truncation_model_fieldhalos='TRUNCATION_RN', kwargs_truncation_model_fieldhalos = {}, mass_range_is_bound = True, proj_angle_theta = 0, proj_angle_phi = 0, preset_model_los = 'CDM', geometry_type = 'DOUBLE_CONE', kwargs_cosmo = None, **kwargs_los):
    ...
    lens_redshifts = np.array([0.34] * len(massInfall))
    #lens_redshifts = 0.34
    r3d_kpc = MPC_TO_KPC * data[4][0]
    rt = data[5][0]
    x_kpc = MPC_TO_KPC * data[6][0]
    y_kpc = MPC_TO_KPC * data[7][0]
    ...
    for n,m_infall in enumerate(massInfall):
        tnfw_args = {key:val[n] for key,val in tnfw_args_all.items()} 
        halo_list.append(TNFWFromParams(m_infall,x_kpc[n], y_kpc[n], r3d_kpc[n],z_lens,True,lens_cosmo,tnfw_args))
