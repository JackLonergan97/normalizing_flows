...
def emulator_data(emulator, lines, num_iterations):
    ...
    N = int(len(massInfall[subhalos])/countTree) # use when we want emulator realization to contain same number of points as Galacticus realization
...
for model, color in zip(dm_models, colors):
    ...
    subsample = np.random.choice(i, size=len(massInfall), replace=True) # include p = w/np.sum(w)) if you want subsampling

    #em_massInfall = np.array(em_massInfall[subsample])
    #em_massBound = np.array(em_massBound[subsample])
    ...
    print('length of em_weights: ' + str(len(em_weights)))
    
    axes.hist(w, bins = 70, range = (0,1000), label = 'Galacticus', fill = False, edgecolor = 'blue')
    axes.hist(em_weights, bins = 70, range = (0,1000), label = 'Emulator', fill = False, edgecolor = 'orange')
    axes.set(title = 'subhalo weights')
    axes.legend()
    plt.savefig('plots/histograms_compare_weights.png')

    f, axes = plt.subplots(1)
    f.set_size_inches(15, 20)

    print('minimum of weightNormalized: ' + str(np.min(weightNormalized)))
    print('maximum of weightNormalized: ' + str(np.max(weightNormalized)))
    print('minimum of em_weights: ' + str(np.min(em_weights)))
    print('maximum of em_weights: ' + str(np.max(em_weights)))

    final_gal = int(len(w)/countTree)
    final_em = int(len(em_weights)/num_iterations)

    print('length of gal_mass_infall(subhalos): ' + str(len(massInfall[subhalos])))
    print('length of gal_mass_bound(subhalos): ' + str(len(massBound[subhalos])))
    print('length of em_mass_infall(subhalos): ' + str(len(em_massInfall)))
    print('length of em_mass_bound(subhalos): ' + str(len(em_massBound)))   

    axes.hist(massInfall[subhalos], weights=w, bins = 70, range = (1e6,1e11), label = 'Galacticus', fill = False, edgecolor = 'blue')
    axes.hist(em_massInfall, weights=w, bins = 70, range = (1e6,1e11), label = 'Emulator', fill = False, edgecolor = 'orange')
    axes.set(title = 'infall mass')
    axes.legend()
    plt.savefig('plots/histograms_compare_infall_masses_w.png')

    f, axes = plt.subplots(1)
    f.set_size_inches(15, 20)

    axes.hist(massBound[subhalos], weights=w, bins = 70, range = (1e6,1e10), label = 'Galacticus', fill = False, edgecolor = 'blue')
    axes.hist(em_massBound, weights=w, bins = 70, range = (1e6,1e10), label = 'Emulator', fill = False, edgecolor = 'orange')
    axes.set(title = 'bound mass')
    axes.legend()
    plt.savefig('plots/histograms_compare_bound_masses_w.png')
